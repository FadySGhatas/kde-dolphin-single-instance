...
extern "C" Q_DECL_EXPORT int kdemain(int argc, char **argv) {
  ...
  parser.addOption(QCommandLineOption(
      ...
      i18nc("@info:shell", "The files and folders passed as arguments "
                           "will be selected.")));
  parser.addOption(QCommandLineOption(
      QStringList() << QStringLiteral("split"),
      i18nc("@info:shell", "Dolphin will get started with a split view.")));
  parser.addOption(QCommandLineOption(
  ...
  parser.addPositionalArgument(QStringLiteral("+[Url]"),
                               i18nc("@info:shell", "Document to open"));
  ...
  const bool splitView =
      parser.isSet(QStringLiteral("split")) || GeneralSettings::splitView();
  ...
  if (app.isSessionRestored()) {
    ...
    if (className == QLatin1String("DolphinMainWindow")) {
      ...
      qCWarning(DolphinDebug) << "Unknown class " << className
